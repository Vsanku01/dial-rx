...
user_input = st.sidebar.text_input("Enter your diagnosis..")
if st.sidebar.button("Send"):
    with st.spinner('Wait for it...'):
        ...
        if response.ok:
            data = response.json()
            st.session_state.previous_answers.append(data['answer'])
...
# Dropdown list for previous answers
if st.session_state.previous_answers:
    selected_answer_index = st.sidebar.selectbox(
        "Previous Answers", options=st.session_state.previous_answers, index=len(st.session_state.previous_answers) - 1)
